...
const App = () => {
  ...
  const [activeStartCell, setactiveStartCell] = useState("");
  const [activeEndCell, setactiveEndCell] = useState("");
  ...
  const handleMouseEnter = (event) => {
    if (event.target.id === "cell") {
      ...
      if (isMouseDown === true) {
        setactiveEndCell(event.currentTarget.dataset.index);
        console.log(
          "Activestart/activeend is: ",
          activeStartCell,
          activeEndCell
        );
      }
  ...
  const handleMouseDown = (event) => {
    ...
    setisMouseDown(true);
    if (event.target.id === "cell") {
      setactiveStartCell(event.currentTarget.dataset.index);
      setactiveEndCell(event.currentTarget.dataset.index);
      console.log(activeStartCell, activeEndCell);
    }
  };

  //Handle updating the DOM when data is inputted
  useEffect(() => {
    if (colorPicker) {
      for (let i = activeStartCell; i <= activeEndCell; i++) {
        addPartialData(colorData, colorPicker.value, i, 0, 0, 99);
      }
      //console.log(`Start Cell: ${activeStartCell}, End Cell: ${activeEndCell}`);
      //console.log(colorData);
    }
  }, [activeStartCell, activeEndCell, colorPicker]);
  ...
  /*====================
    FILL COLORDATA ARRAY
    ====================*/

  const fillColorData = () => {
    console.log("FILLING COLOUR DATA");
  ...
  const renderCells = () => {
    ...
    if (minuteinput !== 0) {
      fillColorData();
      //MOVE THE DATA FROM COLORDATA ARRAY TO CELLS ARAY
